function [M, c, W, update, xi] = getIterationData(H, h, N, dims, xi, varargin)
    ...
    p.addParameter('method', 'lim', @(s)(any(strcmp(s, {'lim', 'admm'}))));
    p.parse(varargin{:});
    options = p.Results;
    ...
    switch options.method
        case 'lim',
            ...
            update = @(hn,b) (xi*R-eye(size(R)))\(R*(h+hn+H*(Aeq\b))-(Aeq\b));  
        case 'admm'
            K = [H+xi*eye(size(H)) Aeq'; Aeq zeros(size(Aeq,1),size(Aeq,1))]^(-1);
            R = K(1:dims.nn,1:dims.nn);
            M = R;
            c = -R*(h+(H+xi*eye(size(H)))*v)+v;
            W = xi*eye(size(M));
            ...
            % Update function for changing h
            update = @(hn,b) (-R*(h+hn+(H+xi*eye(size(H)))*(Aeq\b))+(Aeq\b));
    end
